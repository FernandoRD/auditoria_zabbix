...
The GUI has three action buttons in `main_view.py`'s `control_frame`: **"▶ Iniciar Auditoria"** (fresh collection + AI), **"🔄 Regerar (Apenas IA)"** (reuses `last_audit_cache.json`, skips collection), and **"📥 Apenas Coleta"** (collection only, no AI — prompts a save path and writes the JSON there via `Controller.start_collection_only`/`run_collection_only_flow`). The actual connect+collect+anonymize+cache-write logic lives in one shared helper, `Controller._collect_zabbix_data()`, used by both `run_audit_flow` (non-cache path) and `run_collection_only_flow` — keep new collection logic there so it doesn't drift between the two entry points.
